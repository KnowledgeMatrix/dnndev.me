...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		html +=
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;background-color:" + event.eventColor +";z-index:8;left:"+left+"px'>" +
...
function Agenda(element, options, methods) {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			html +=
				"<div class='" + className + event.className.join(' ') + "' style='position:absolute;background-color:"+ event.eventColor + ";z-index:8;top:" + top + "px;left:" + left + "px'>" +
					"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
