<%@ control language="C#" autoeventwireup="true" inherits="EasyDNN.Modules.EasyDNNGallery.DesktopModules_EasyDNNGallery_Controls_VideoGallery, App_Web_videogallery.ascx.dc9a219e" %>
<style type="text/css">
	.titleempty {
		display: none;
	}
								...
								<div class="EDGcornertopright">
									<div class="EDGcontent" style="text-align: center;">
										...
										<asp:DataList ID="dlVideos" runat="server" Font-Bold="False" Font-Italic="False"
											...
											OnItemCreated="dlVideos_ItemCreated"
											...
											<ItemTemplate>
												<asp:PlaceHolder ID="plhBitsOnTheRun" runat="server" Visible='<%# ToDisplayBits("Embeded Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info")) %>'>
													<div id="Div1" style="z-index: 0">
														...
														<div class="VideoBodyEmbed">
															<script src="<%# GetBitsUrl(Eval("ShortEmbedUrl"))%>" type="text/javascript"></script>
														</div>
												...
												<asp:PlaceHolder ID="phSWfFlash" runat="server" Visible='<%# ToDisplayVideo("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName")) %>'>
													...
													<div class="VideoBodySwf">
														<div id='<%#Eval("PictureID")%>'>
															You need to upgrade your flash player.
														</div>
													</div>
													<div class="VideoDescription">
														<asp:Label ID="lblVGEVMediaDescription3" runat="server" Text='<%# ShowDescription(DataBinder.Eval(Container.DataItem, "Description"))%>'></asp:Label>
													</div>
													<script type="text/javascript">
															<%=jQuery%>(document).ready(function () {
																var flashvars = {};
												...
												<asp:PlaceHolder ID="phEVVideo" runat="server" Visible='<%# ToDisplayVideo2Flow("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName"),"Normal") %>'>
													...
													<%=jQuery%>(document).ready(function () {
														...
														flashvars.skin = '<% =moduleSettings.VGSkin%>'
														flashvars.fullscreenbutton = '<%=AllowFullscren%>'
														flashvars.infobutton = '<%=moduleSettings.InfoButton%>'
														if ('<%=moduleSettings.StartPhoto%>' == 'True') {
														...
														flashvars.initialvolume = '<%=moduleSettings.InitialVolume%>'
														...
														attributes.id = 'pp'
														/* ]]> */
														swfobject.embedSWF('<%=ModulePath.Replace("Controls/","")%>preview.swf', '<%#"Mid" + ModuleId.ToString() %>', '<%#GetVideoWidth(DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "FileName"))%>', '<%# GetVideoHeight(DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "FileName"))%>', '9.0.0', '<%=ModulePath.Replace("Controls/","")%>js/expressInstall.swf', flashvars, params, attributes);
													});
													</script>
												...
												<asp:PlaceHolder ID="phEVVideoFlow" runat="server" Visible='<%# ToDisplayVideo2Flow("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName"),"Flow") %>'>
														...
														<div class="VideoTitle">
															<asp:Label ID="lblVideoFlowTitle" runat="server" CssClass="EDGTitleText" Text='<%#ShowTitle(DataBinder.Eval(Container.DataItem, "Title"))%>' Visible="<%#Convert.ToBoolean(moduleSettings.ShowMediaTitle)%>"></asp:Label>
														</div>
														<div class="VideoBodyPlayer">
															<div style='display: block; width: <%#GetFlowVideoWidth(DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "FileName"))%>px; height: <%# GetFlowVideoHeight(DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "FileName"))%>px' class="flowplayer" fullscreen="<%= moduleSettings.AllowFullscren.ToLower()%>" data-tooltip="false" data-flashfit="true" data-native_fullscreen="true" data-adaptiveratio="false" controls="controls" preload="metadata" data-volume="<%=moduleSettings.InitialVolume%>">
																<video <%=(moduleSettings.PlayOnLoad=="True"?"autoplay":"") %> <%=loopFlow%> <%# GetFlowPoster(DataBinder.Eval(Container.DataItem, "ThumbUrl"))%>>
																	<source type="video/mp4" src="<%#GetFlowFileName(DataBinder.Eval(Container.DataItem, "FileName"))%>">
																</video>
															</div>
														</div>
														<div class="VideoDescription">
															<asp:Label ID="lblVGEVMediaDescription2Flow" runat="server" Text='<%# ShowDescription(DataBinder.Eval(Container.DataItem, "Description"))%>' Visible="<%#Convert.ToBoolean(moduleSettings.ShowMediaDescription)%>"></asp:Label>
														</div>
													<div ><br /></div>
												</asp:PlaceHolder>
												<asp:PlaceHolder ID="phHtml5Video" runat="server"
													...
													<div class="VideoTitle">
														<asp:Label ID="Label1" runat="server" CssClass="EDGTitleText"
															Text='<%#ShowTitle(DataBinder.Eval(Container.DataItem, "Title"))%>'></asp:Label>
													</div>
													<div class="VideoDescription">
														<asp:Label ID="lblVGEVMediaDescription5" runat="server"
															Text='<%# ShowDescription(DataBinder.Eval(Container.DataItem, "Description"))%>'></asp:Label>
													</div>
													<style type="text/css">
													#Div<%#Eval("PictureID")%>video_player 
													{
													...
													<div id="Div<%#Eval("PictureID")%>video_player">
													</div>
													<script type="text/javascript">
													<%=jQuery%>(document).ready(function ($) {
														$('#Div<%#Eval("PictureID")%>video_player').H5Video({
															events: {
																pause: function () {
															...
															preload: true,
															autoPlay: false,
															poster: '<%#DataBinder.Eval(Container.DataItem, "ThumbUrl")%>',
