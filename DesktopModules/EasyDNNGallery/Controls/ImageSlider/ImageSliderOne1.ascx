...
{
	$('#<%=ModulNr%> a, #<%=ModulNr%>EasyDNNGallery .EDGvideo a[title], #<%=ModulNr%>EasyDNNGallery .EDGaudio a[title]').click(false);
	$('#<%=ModulNr%>').nivoSlider({
		...
		<div class="<%=slideBorder2%>" style="<%=nivoTheme%>">
			<div id="<%=ModulNr%>" style="<%=imageItemstyle%>">
					<asp:Repeater ID="repImageSlider" runat="server" DataSourceID="odsGetOnlyImages">
						<ItemTemplate>
							<a href="#" title='<%#GetDescription3(DataBinder.Eval(Container.DataItem, "Description"),DataBinder.Eval(Container.DataItem, "PictureID"))%>' style="cursor:default">
								<img id="imgImageSlider" alt='<%#GetAlt(Eval("Title"),Eval("PictureID"))%>' src='<%#GetThumbPath(DataBinder.Eval(Container.DataItem, "ViewerThumb")) %>' title='<%#ImageSliderTD(Eval("Title"),Eval("Description"),Eval("PictureID")) %>' />
							</a>
						</ItemTemplate>
					</asp:Repeater>
				</div>
