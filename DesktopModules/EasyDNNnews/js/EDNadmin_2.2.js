eds2_2(document).ready(function ($) {
	...
	$edNews_adminWrapper = $('.edNews_adminWrapper'),
		$body = $('body'),
		$form = $('> form', $body),
		$contentSection = $edNews_adminWrapper.find('div.contentSection'),
		...
		google_maps_token = function (token) {
			var $window = $(window),
						...
						if (edit_marker) {
							...
							google.maps.event.addListener(the_marker, 'mouseover', function () {
								for (var i = 0, l = map_markers.length; i < l; i++) {
									if (map_markers[i].marker.get('id') == this.get('id')) {
					...
					if (editing_mode) {
						for (var i = 0, l = map_markers.length; i < l; i++) {
							...
							google.maps.event.addListener(marker, 'mouseover', function () {
								for (var j = 0, l = map_markers.length; j < l; j++) {
									if (map_markers[j].marker.get('id') == this.get('id')) {
										$map_markers.find('.marker').eq(m).addClass('hover');
				...
				close_dialog = function () {
					...
					$the_map.remove();
					google.maps.event.clearInstanceListeners(window.edn_map_token_obj);
					...
					if (map_markers.length) {
						...
						for (var i = 0, l = map_markers.length; i < l; i++) {
							markers_html += '<div class="marker">'+
								'<input type="checkbox" name="check_marker" value="1" /><a href="#" class="marker_title">' + map_markers[i]._meta.title + '</a> - <a href="#" class="edit">' + localization.gmaps_token_editor_edit_marker + '</a>'+
								'</div>';
						};
				...
				show_token_area = function () {
					var tokenString = generate_token();

					$google_maps.data('token_area_active', true);

					$the_token.text(tokenString);
			...
			if (editing_mode) {
				(function () {
					...
					for (var i = 0, l = token_parts.length; i < l; i++) {
					...
					if (markers_string != '') {
						...
						for (var i = 0, l = markers.length; i < l; i++) {
							...
							marker_settings = {
								latitude: lat_lng[0],
								longitude: lat_lng[1],
								title: decode_from_token(value_pairs[1]),
								html: decode_from_token(value_pairs[2])
							};
	...
	$contentSection.delegate('.edNews__toggleWrapper > .edNews__toggle', 'click', function () {
		var $toggle = $(this),
			$toggleContent = $toggle.parent().siblings('.edNews__toggledContent'),
			$toggleContentState = $toggleContent.find('> input[type="hidden"]');

		if ($toggle.hasClass('edNews__open')) {
			$toggle.removeClass('edNews__open');
			$toggleContent.slideUp(200);
			$toggleContentState.val('closed');
		} else {
			$toggle.addClass('edNews__open');
			$toggleContent.slideDown(200);
			$toggleContentState.val('open');
		}
	...
	$module_settings
		.delegate('.category_toggle > .edNews_inputGroup', 'click', function (e) {
		...
		.delegate('.category_toggle', 'click', function () {
			...
			if ($category_content.is(':visible')) {
				$category_state.val('edNews__collapsed');
				$category_content.slideUp(200, function () {
					$parent.addClass('edNews__collapsed');
				});
			} else {
				$category_state.val('');
				$parent.addClass('opening');
				$category_content.slideDown(200, function () {
					$parent.removeClass('opening edNews__collapsed');
				});
			}
		})
		.delegate('.edNews_settingsCollapse a', 'click', function () {
			var $clicked = $(this),
				$all_cats = $clicked.parent().parent().next().children().siblings('.settings_category_container'),
				$all_cats_content = $all_cats.find('> .category_content'),
				$all_cats_state = $all_cats.find('> input[type="hidden"]');

			if ($clicked.hasClass('edNews__expand')) {
				$clicked.parent().removeClass('edNews__collapsed');
			}
			else {
				$clicked.parent().addClass('edNews__collapsed');
			}

			if ($clicked.hasClass('edNews__collapse')) {
				$all_cats.addClass('edNews__collapsed');
				$all_cats_state.val('edNews__collapsed');
				$all_cats_content.css('display', '');
			} else {
				$all_cats.removeClass('edNews__collapsed');
				$all_cats_state.val('');
	...
	$('.edNews__visibleRequiredToggles', $edn_admin).on('change', 'input[type="checkbox"]', function () {
		var $this = $(this);

		if (this.id.indexOf('Required') == -1) {
			if (!this.checked)
				$this.parent().parent().siblings('.switchCheckbox').find('input:checkbox').prop('checked', false);
		} else {
			if (this.checked)
				$this.parent().parent().siblings('.switchCheckbox').find('input:checkbox').prop('checked', true);
		}
	});

	if (typeof EasyDnnSolutions1_1_instance != 'undefined')
		EasyDnnSolutions1_1_instance.google.maps.api.load();
});
