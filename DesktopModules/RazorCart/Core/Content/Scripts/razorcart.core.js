(function () {
    ...
    angular.module('RazorCart', ['ngProvider', 'ngMessages', 'toastr'])
        ...
        .run(function ($rootScope, $location, $window, $rootElement, $timeout, globalFunctions) {
            ...
            $rootScope.$watch(function () { return $location.absUrl() }, function (newLocation, oldLocation) {
                if (false && $rootScope.actualLocation && newLocation.toLowerCase() === $rootScope.actualLocation.toLowerCase()) {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    globalFunctions.$inject = ['$location', '$rootScope'];
    function globalFunctions($location, $rootScope) {
        ...
        var updateUrlLocation = function (goToFirstPage, params) {
            ...
            var strParams = '';
            var paramPairs = [];
            angular.forEach(pathParams.params, function (ngValue, ngKey) {
                //paramPairs.push(ngKey + (productListInfo.useFriendlyUrl ? '/' : '=') + ngValue);
                paramPairs.push(ngKey + '=' + encodeURIComponent(ngValue));
            });
            if (paramPairs.length) {
                //strParams = (productListInfo.useFriendlyUrl ? '/' : '?') + paramPairs.join(productListInfo.useFriendlyUrl ? '/' : '&');
                strParams = '?' + paramPairs.join('&');
            }
            var newUrl = productListInfo.absoluteBaseUrl + strParams;
            pathParams.path = newUrl;
            sharedService.initPathParams(pathParams.path, pathParams.params);
            //var index = urlParams.map(function (pathParams) {
            //    return pathParams.path.toLowerCase();
            //}).indexOf(newUrl.toLowerCase());
            //if (index > -1) {
            //    urlParams[index] = pathParams;
            //} else {
            //    urlParams.push(pathParams);
            //}
            if (window.history && window.history.pushState) {
                var domain = location.protocol + "//" + location.host;
                $location.url(newUrl.replace(new RegExp(domain, 'i'), ''));
            } else {
    ...
    function dataProvider($http) {
        ...
        var updateShoppingCart = function (moduleId, updateList) {
            var servicesFramework = $.ServicesFramework(moduleId);
            return $http.put(servicesFramework.getServiceRoot('RazorCart') + 'Cart/Update', updateList, {
                headers: { 'Content-Type': 'application/json; charset=utf-8', 'ModuleId': servicesFramework.getModuleId(), 'TabId': servicesFramework.getTabId(), 'RequestVerificationToken': servicesFramework.getAntiForgeryValue() }
            }).then(function (response) {
                return response;
            }).catch(function (error) {
        ...
        var saveCustomer = function (moduleId, shippingData) {
            var servicesFramework = $.ServicesFramework(moduleId);
            return $http.post(servicesFramework.getServiceRoot('RazorCart') + 'Customer/Save', { ShippingData: shippingData }, {
                headers: { 'Content-Type': 'application/json; charset=utf-8', 'ModuleId': servicesFramework.getModuleId(), 'TabId': servicesFramework.getTabId(), 'RequestVerificationToken': servicesFramework.getAntiForgeryValue() }
            }).then(function (response) {
                return response;
            }).catch(function (error) {
