(function () {
    var MentionsBase, MentionsContenteditable, MentionsInput, Selection, entityMap, escapeHtml, escapeRegExp, namespace, getRedactorIfDefined,
    ...
    $.widget("ui.areacomplete", $.ui.autocomplete, {
        ...
        _renderItem: function (ul, item) {
            ...
            $(ul).addClass('evoq-autocomplete-menu');
            li = $('<li class="evoq-autocomplete-menu-item"></li>');
            anchor = $('<a>').appendTo(li);
            if (item.image) {
                anchor.append("<img src=\"" + item.image + "\" />");
            }
            regexp = new RegExp("(" + escapeRegExp(this.searchTerm) + ")", "gi");
            value = item.value.replace(regexp, "<a>$&</a>");
            anchor.append(value);
            return li.appendTo(ul);
        }
    });

    getRedactorIfDefined = function(element) {
        if (element && element.data() && element.data().mentionsInput && 
            element.data().mentionsInput.options && element.data().mentionsInput.options.redactor) {

            return element.data().mentionsInput.options.redactor;
        }
        return null;
    };
        ...
        selectCallback: function (event, ui) {
            ...
            if (this.overriden.select) {
                ...
                if (this.overriden.select(event, ui) === false) {
                    var redactor = getRedactorIfDefined(this.element);
    ...
    MentionsInput = (function (superClass) {
        ...
        MentionsInput.prototype._updateValue = function () {
            ...
            for (j = 0, len = ref.length; j < len; j++) {
                ...
                hlContent.push("<a>" + mention.name + "</a>");
    ...
    MentionsContenteditable = (function (superClass) {
        var insertMention, mentionTpl, lastTextNode;
        ...
        function MentionsContenteditable(input1, options) {
            ...
            this._initEvents();
            $(this.input).on('keypress keyup', function() {
        ...
        mentionTpl = function (mention) {
            return "<a class=\"evoq-user-mention\" data-mention=\"" + mention.uid + "\">" + mention.value + "</a>";
        };
