...
﻿var PageHistoryVersionModel = (function ($, ko) {
    ...
    var PageHistoryVersionModel = function () {
        ...
        this.tabVersionCompareCss = '<style type="text/css">' +
                'ins {background-color: rgba(76, 255, 0, 0.65); text-decoration: none;} ' +
                'ins > img { opacity: .5; filter: Alpha(Opacity=50);}' +
                'del {background-color: rgba(255, 194, 200, 0.8); text-decoration: none;}' +
        ...
        this.showVersion = function (version) {
            self.request('EvoqContentLibrary', 'Versions', 'DoesVersionExist', 'post', { Version: version.versionNumber }, function (data) {
                if (!data.Exists) {
                    $.dnnAlert({
                        title: "",
                        text: data.Message
        ...
        this.ensureComparision = function () {
            var results = self._getSelectedVersions();
            var newVersionNumber = results[0].versionNumber;
            var previousVersionNumber = results[1].versionNumber;
            var doesVersionExist = true;
            var message = "";
            $.when(self.request('EvoqContentLibrary', 'Versions', 'DoesVersionExist', 'post', { Version: newVersionNumber }, function(data) {
                    if (!data.Exists) {
                ...
                self.request('EvoqContentLibrary', 'Versions', 'DoesVersionExist', 'post', { Version: previousVersionNumber }, function (data) {
                    if (!data.Exists) {
                        doesVersionExist = false;
                        message = data.Message;
                    }
                ...
                self.compareVersions(newVersionNumber, previousVersionNumber);
            });
        }

        this.compareVersions = function (newVersionNumber, previousVersionNumber) {
            var content1, content2, previousVersion, newVersion, setCompareProcessAsFailed, hasCompareProcessFailed = false;
            var urlVersion = self._getIFrameUrl(newVersionNumber);
            var urlPreviousVersion = self._getIFrameUrl(previousVersionNumber);

            self._updateVersionsInfo(newVersionNumber, previousVersionNumber);
            ...
            $.when(
                ...
                self.request('EvoqContentLibrary', 'Versions', 'ComparePage', 'post', { 'content1': $content2.html(), 'content2': $content1.html() }, function (data) {
                    ...
                    //fix diff content to remove DnnEmptyPane class when the pane contains deleted modules
                    self._correctEmptyPaneStats($htmlDiff);
        ...
        this._correctEmptyPaneStats = function(container) {
            container.find('.DNNEmptyPane').each(function() {
                var $pane = $(this);
                if ($pane.children('div.DnnModule').length) {
                    $pane.removeClass('DNNEmptyPane');
                }
            });
        };
