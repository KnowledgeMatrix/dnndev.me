...
dnn.EditBar = (function ($) {
    ...
    var initAddExistingModuleButton = function () {
        var $button = $('#edit-bar .AddExistingModule');
        $button.click(function () {
            var dialog = dnn.ContentEditorManager.getExsitingModuleDialog();
            var contentPane = dialog.getDefaultPane();
            var moduleManager = contentPane.data('dnnModuleManager');
            if (moduleManager) {
    ...
    var initPageSettingsButton = function () {
        ...
        if (dnn.getVar('cem_pagetype') === 'template' || dnn.getVar('cem_isAllowedToOpenSettings') === 'False') {
            $button.hide();
        } else {
            $button.click(function() {
    ...
    var pageEditComplete = function pageEditComplete() {
        if (dnn.PersonaBar) {
            dnn.PersonaBar.offPanelOpen(pageEditComplete);
        }
    ...
    var openPageSetting = function openPageSetting(view, params) {
        if (dnn.PersonaBar) {
            dnn.PersonaBar.openPanel('Dnn.Pages', { viewName: view, viewParams: params });
            dnn.PersonaBar.onPanelOpen(pageEditComplete);
        }
    };

    var EditBar = function (resxSettings) {
        var self = this;
        ...
        this._getPageVersions = function (cb) {
            request('EvoqContentLibrary', 'Versions', 'GetPageVersions', 'get', { },
                function (data) {
        ...
        this._pageContentChanged = function () {
            var params = { };
            request('EvoqContentLibrary', 'PageManagement', 'DeleteThumbnails', 'POST', params, function (data) {
            }, function () {
            }, true);
        };
        ...
        var resxCallback200 = function(response) {
            resx = response.localization;

            self._workflowManager = new window.dnn.pageWorkflow.PageWorkflowManager(request, self._pageHistoryManager, resx, getService(), (dnn.getVar('dnn_history_hasPendingChanges') === 'true'));
        ...
        this.initUI = function () {
            ...
            var isPersonaBarShown = dnn.getVar('cem_personaBarShown').toLowerCase();
            if (isPersonaBarShown === "true") {
                $('#editBarContainer').addClass('personaBarShown');
            }

            initAddModuleButton();
            initAddExistingModuleButton();
        ...
        }).on("personabar:show", function onPersonaBarOpened() {
...
$(window).load(function () {
    $.get(dnn.getVar('cem_resourceroot') + '/EditBar/Html/edit-bar.html', function(data) {
        $("#Body").append(data);
        var resxSettings = {
            culture: dnn.getVar('cem_editBar_resx_culture'),
            resxTimeStamp: dnn.getVar('cem_editBar_resx_timestamp')
        };
        var editBar = window['CEM_EditBar'] = new dnn.EditBar(resxSettings);
        editBar.initUI();
