...
(function ($) {
    ...
    // Plugin
    $.fn.dnnRedactor = function (options) {
        var val = [];
    ...
    // Functionality
    $.dnnRedactor = Redactor;
    $.dnnRedactor.VERSION = '10.2';
    $.dnnRedactor.modules = ['alignment', 'autosave', 'block', 'buffer', 'build', 'button',
    ...
    $.dnnRedactor.opts = {
        ...
        langs: {
            en: {
                ...
                // DNN Customized: needed for Engage, since Redactor 9 had this literal.
                exceedLimit: 'This file is above the {0} Mb limit, please upload a smaller file.'
                // END customization.
            }
        },
    ...
    // Functionality
    Redactor.fn = $.dnnRedactor.prototype = {
        ...
        loadModules: function () {
            var len = $.dnnRedactor.modules.length;
            for (var i = 0; i < len; i++) {
                this.bindModuleMethods($.dnnRedactor.modules[i]);
            }
        },
        ...
        build: function () {
            return {
                ...
                createTextarea: function () {
                    this.$textarea = $('<textarea />').attr('name', this.build.getTextareaName()).attr('aria-label', 'Content');
                },
                ...
                setEvents: function () {
                	...
                	// DNN Customised
                    /* CONTENT-5141:
                        In IE drag files into redactor doesn't work for modules that are edited in View Mode (all the Engage ones and Publisher).
                        To fix this, this event has to be customized.
                    */
	                this.$editor.on('dragover.redactor', $.proxy(function(e) {
						e.stopPropagation();
						e.preventDefault();
	                }, this));
					// END Customised
                    ...
                    // DNN Customised
                    if (!this._documentClickEventHandler) {
                        this._documentClickEventHandler = $.proxy(function (e) {
                            clickedElement = $(e.target);

                            var blockBlur = clickedElement.parents('.redactor-blockBlur').size() > 0
		                        || clickedElement.parents('.redactor-box').size() > 0
		                        || clickedElement.attr('id') == 'redactor-modal-overlay';
                            if (!blockBlur) {
                                this.$editor.triggerHandler('blur.redactor');
                            }
                        }, this);
                    }
                    $(document).mousedown(this._documentClickEventHandler);
                    // END Customised
                    ...
                    this.$editor.on('blur.redactor', $.proxy(function (e) {
                        ...
                        if (!$clickedElement.hasClass('redactor-toolbar')
		                        && $clickedElement.parents('.redactor-toolbar').size() == 0
		                        && $clickedElement.parents('.redactor-blockBlur').size() == 0
		                        && $clickedElement.parents('.redactor-box').size() == 0
		                        && $clickedElement.attr('id') != 'redactor-modal-overlay') {

                            if (this.rtePaste) return;

                            this.blurClickedElement = clickedElement; 
        ...
        code: function () {
            return {
                ...
                // DNN customised - function used by Enchage for clean code get
        ...
        core: function () {
            return {
                ...
                destroy: function () {
                    ...
                    // DNN Customised
                    if (this._documentClickEventHandler) {
                        $(document).off('mousedown', this._documentClickEventHandler);
                    }
                    // END Customised
        ...
        image: function () {
            return {
                ...
                showEdit: function ($image) {
                    ...
                    /* DNN Customization: SOCIAL-4149 
                        title MUST be read from title, since alt is the text 
                        shown when the image can't be shown. */
                    $('#redactor-image-title').val($image.attr('title'));
                    /* DNN Customization END */
                ...
                update: function ($image) {
                    ...
                    /* DNN Customization: SOCIAL-4149 
                        title MUST be also save in the title attribute. */
                    $image.attr('title', $('#redactor-image-title').val());
                    /* DNN Customization ends. */
                ...
                setEditable: function ($image) {
                    ...
                    var handler = $.proxy(function (e) {
                        ...
                        // DNN Customised, if return false, the callbacks added by ImageEditing weren't executed.
            ...
            return {
                ...
                nodeToCaretPositionFromPoint: function (e, node) {
                    ...
                    if (document.caretPositionFromPoint) {
                        ...
                        /* DNN customized: CONTENT-5141
                           Original source code doesn't work properly and produces
                           errors. To avoid this the focus on where to put the image
                           is set by using the Redactor API.
                           This also takes into account that in some scenarios the 
                           selected element is one involved in the images manipulation. In
                           this case, this element is not selected since will be deleted.
                           */
                        var IMAGE_WRAPPER_CSS_CLASS = 'dnnImageWrapper';
                        var selectionResult = this.selection.getCurrent();
                        ...
                        var isAnImageWrapperThatWillBeDeleted = $(selectionResult).hasClass(IMAGE_WRAPPER_CSS_CLASS);
                        if (isAnImageWrapperThatWillBeDeleted) {
                            try {
                                selectionResult = selectionResult.parentNode;
                                this.caret.setBefore(selectionResult);
                            } catch (error) {
                                this.focus.setEnd();
                            }
                        }
                        /* DNN Customziation END */
                    }
                },
        ...
        modal: function () {
            return {
                ...
                /* DNN Customization:
                        CONTENT-5092 - Changed styles to avoid that some elements
                        be hidden by the PB. */ 
                resizeWithPersonaBar: function(){
                    var PERSONA_BAR_WIDTH = 85,
                        PERSONA_BAR_IFRAME_ID = 'personaBar-iframe',
                        PERSONA_BAR_ID = 'personabar';

                    var personaBarIframe = document.getElementById(PERSONA_BAR_IFRAME_ID);
                
                    if (personaBarIframe 
                            && personaBarIframe.contentWindow.document.getElementById(PERSONA_BAR_ID)){
                        var windowWidth = $(window).width();
                        var noWidthEnough = this.$modal.width() > windowWidth - PERSONA_BAR_WIDTH;

                        if (noWidthEnough) {
                            this.$modal.css({
                                width: this.$modal.width() - PERSONA_BAR_WIDTH
                            });
                        }

                        this.$modal.css({
                            left: Math.floor(PERSONA_BAR_WIDTH / 2)
                        });
                    }
                },
                /* DNN Customization END */
                show: function () {
                    ...
                    // resize
                    /* DNN Customization - CONTENT-5092:
                        It's necessary to adapt the styles if the Persona Bar is shown. */
                    if (!this.utils.isMobile()) {
                        setTimeout($.proxy(function (){
                            this.modal.showOnDesktop();
                            this.modal.resizeWithPersonaBar();
                        }, this), 0);
        ...
        toolbar: function () {
            return {
						    ...
						    dropdown:
							{
							    link:
								{
								    ...
								    observe: {
								        ...
								        //DNN Customized: 
								        /* SOCIAL-4264:
                                            The below in property should be specified as text due to IE8 considers it a reserved word
                                        */
								        "in": {
								            title: this.lang.get('link_edit')//, 
                                            //DNN Customized: 
								            /* SOCIAL-4264:
                                                The above comma provokes some errors in IE8
                                            */
								        },
								        out: {
								            title: this.lang.get('link_insert')
                ...
                setFixed: function () {
					//DNN Customized: 
					/* CONTENT-5133:
						Allows redactor toolbar to stick on mobile.
					*/
                ...
                observeScrollEnable: function (scrollTop, boxTop) {
                    // DNN Customized: 
                    /* CONTENT-5133:
                        In the Publisher Module, the redactor toolbar fails to take into account the Host's
                        control bar that is fixed to the top of the screen. This fix is used for when the SuperUser
                        is logged in and the ControlBar is visible.
                        --ADDED: + $("#ControlBar").outerHeight() to var top = this.opts.toolbarFixedTopOffset + scrollTop - boxTop
                    */
                    var top = this.opts.toolbarFixedTopOffset + scrollTop - boxTop + $("#ControlBar").outerHeight();
                    var left = 0;
        ...
        utils: function () {
            return {
                ...
                isRedactorParent: function (el) {
                    ...
                    // DNN Customized: 
                    /* CONTENT-5095:
                        The image editing toolbar are attached to the image, this implies that some of their elements
                        have Redactor as parent. However, these elements can be handled as text elements in the Redactor body
                        since this provokes that Redactor manipulates them and replaces them by other elements regarding 
                        its configuration.
                    */
                    if ($(el).parents('#image-edit-bar').length !== 0) {
                        return false;
                    }
                    // DNN Customziation END
    ...
    $(window).on('load.tools.redactor', function () {
        $('[data-tools="redactor"]').dnnRedactor();
    });
