...
<div class="dnnFormItem">
	<dnn1:LabelControl ID="RuleTypeLabelControl" runat="server" Text="Exemption rule:" />
	<asp:DropDownList ID="RuleTypeDropDownList" runat="server" OnSelectedIndexChanged="RuleTypeDropDownList_SelectedIndexChanged" AutoPostBack="True">
		<asp:ListItem Text="None - always tax" Value=""></asp:ListItem>
		<asp:ListItem Text="Basic" Value="BasicExemptionView"></asp:ListItem>
...
<asp:MultiView ID="FormulaMultiView" runat="server" ActiveViewIndex="0">
	<asp:View ID="BasicExemptionView" runat="server">
		<div class="dnnFormItem">
			<dnn1:LabelControl ID="AllowTaxExemptionNumberBasicExemptionLabelControl" runat="server" Text="Use tax exemption number:" HelpText="Exempt user from tax if a tax exemption number is provided." />
			<asp:CheckBox ID="AllowTaxExemptionNumberBasicExemptionCheckBox" runat="server" />
		</div>
		<div class="dnnFormItem">
			<dnn1:LabelControl ID="RegionBehaviorBasicExemptionLabelControl" runat="server" Text="Region match:" HelpText="Determine if matching region is taxable." />
			<asp:RadioButtonList ID="RegionBehaviorBasicExemptionRadioButtonList" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow" CssClass="dnnFormRadioButtons">
				<asp:ListItem Text="Tax all except listed below" Value="exclusion" Selected="True" />
				<asp:ListItem Text="Tax only those listed below" Value="inclusion" />
			</asp:RadioButtonList>
		</div>
		<div class="dnnFormItem">
			<dnn1:LabelControl ID="RegionBasicExemptionLabelControl" runat="server" Text="Regions:" HelpText="Exemption is determined by the customer's shipping address." />
			<div class="rvdFormSubContainer">
				<asp:PlaceHolder ID="MessagePlaceHolder" runat="server" />
				<asp:GridView ID="RegionBasicExemptionGridView" runat="server" CssClass="dnnGrid" GridLines="None" AutoGenerateColumns="False" DataKeyNames="CountryCode,SubdivisionCode,PostalCode" OnRowDeleting="RegionBasicExemptionGridView_RowDeleting" OnSelectedIndexChanged="RegionBasicExemptionGridView_SelectedIndexChanged">
				...
				<div class="rvdActions">
					<asp:LinkButton ID="AddRegionBasicExemptionLinkButton" runat="server" Text="Add new" OnClick="AddRegionBasicExemptionLinkButton_Click" CausesValidation="false" CssClass="dnnPrimaryAction rvdAddNewAction" />
				</div>
				<asp:Panel ID="RegionBasicExemptionDetailsViewPanel" runat="server">
					<div class="dnnFormItem">
						<dnn1:LabelControl ID="CountryCodeBasicExemptionDetailsViewLabelControl" runat="server" Text="Country:" />
						<asp:DropDownList ID="CountryCodeBasicExemptionDetailsViewDropDownList" runat="server" AutoPostBack="True" OnSelectedIndexChanged="CountryCodeBasicExemptionDetailsViewDropDownList_SelectedIndexChanged">
					...
					<div class="dnnFormItem">
						<dnn1:LabelControl ID="SubdivisionCodeBasicExemptionDetailsViewLabelControl" runat="server" Text="State/Province:" />
						<asp:DropDownList ID="SubdivisionCodeBasicExemptionDetailsViewDropDownList" runat="server">
						</asp:DropDownList>
					</div>
					<div class="dnnFormItem">
						<dnn1:LabelControl ID="PostalCodeBasicExemptionDetailsViewLabelControl" runat="server" Text="Postal code:" HelpText="Enter a postal code to match or leave blank to match any postal code. If you enter a substring 902 postal code, it will also match 90210. This allows you to group your postal code into manageable regions." />
						<asp:TextBox ID="PostalCodeBasicExemptionDetailsViewTextBox" runat="server"></asp:TextBox>
					</div>
					<div class="rvdActions">
						<asp:LinkButton ID="SaveRegionBasicExemptionLinkButton" runat="server" Text="OK" OnClick="SaveRegionBasicExemptionLinkButton_Click" ValidationGroup="RegionBasicExemptionDetailsViewControl" CssClass="dnnPrimaryAction rvdOKAction" />
					</div>
				</asp:Panel>
			</div>
		</div>
	...
	<asp:View ID="CustomRuleView" runat="server">
		<div class="dnnFormItem">
			<dnn1:LabelControl ID="ExemptionRuleLabelControl" runat="server" Text="XSLT rule:" />
