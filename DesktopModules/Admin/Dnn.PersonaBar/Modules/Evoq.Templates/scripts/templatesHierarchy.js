...
define(['jquery', 'knockout', 'main/config', 'main/../modules/dnn.pages/scripts/pages.thumbnails'], function ($, ko, cf) {
    var OVER_TIME_TO_OPEN_PAGE_CHILDS;
    var config, templatesHierarchyManager, pageHierarchyDefaultOptions;
    ...
    templatesHierarchyManager = function (options) {
        this.options = options;
    };

    templatesHierarchyManager.prototype = {
        constructor: templatesHierarchyManager,

        init: function ($panel, initCallback, onDeleteCallbck) {
            var handler, viewModel;
            onDeleteCallback = onDeleteCallbck;
        ...
        load: function () {
            dnn.dnnPageThumbnails.init(this.panel);
        ...
        _loadPageTemplates: function (callback) {
            ...
            this._getService().get('GetPageTemplates', params, function (data) {
                ...
                dnn.dnnPageThumbnails.init(handler.panel);
                dnn.dnnPageThumbnails.updateThumbnails();
...
//                handler._getService("EvoqRecyclebin").get('GetDeletedTemplates', {}, function (deletedPages) {
...
//                });
        ...
        _addPagesList: function (parentPage, pagesData) {
            ...
            setTimeout(function () {
                ...
                handler._initScrollView(true, function () {
                    var listScroller, containerScroller, $nextList;
        ...
        _deletePageClickHandler: function (pageData, e) {
            ...
            handler = this;
            viewModel = this._getViewModel();
            confirmText = this.resx.pages_DeleteTemplateConfirm;
            ...
            this.utility.confirm(confirmText, deleteText, cancelText, function () {
                handler._getService().post('DeletePage', { id: pageData.id }, function () {
                    ...
                    if (data && data.responseJSON && data.responseJSON.Message) {
                        ...
                        handler.utility.notifyError("Failed...");
                    }
                });
            });
        },
        ...
        _initScrollView: function (resetContainer, callback) {
            ...
            bottomSpace = 30;
            $pagesList.css('height', 770).each(function () {
            ...
            if (resetContainer) {
                ...
                if(scrollContainer.find(' li').length > 1) {                  
        ...
        _getService: function () {
            ...
            this.utility.sf.controller = "Templates";
    ...
    if (!dnn.dnnTemplatesHierarchy) {
        dnn.dnnTemplatesHierarchy = new templatesHierarchyManager();
    }

    return dnn.dnnTemplatesHierarchy;
});
