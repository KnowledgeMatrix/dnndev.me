...
define(['jquery', 'knockout',
        ...
        'jquery.qatooltip', './koComponents/communityUsers'], function ($, ko, d3, pikaday, moment, d3Charts, cf, pager, scroller) {
    return function() {
        ...
        var updateDonutGraph = function(wrapper, info) {
            ...
            d3.select('[id="' + wrapper.attr('id') + '"] svg')
                .datum(data)
                .call(chart);
        };
        ...
        var updateAdoptionsAndParticipationGraph = function(labels, adoptions, participation, spectators, animation) {
            ...
            var $adoptionsAndParticipations = wrapper.find('.adoptions-and-participation');
            $adoptionsAndParticipations.find('svg').remove();
            $adoptionsAndParticipations.append('<svg class="line-chart"></svg>');
            ...
            var width = $adoptionsAndParticipations[0].clientWidth - 10;
            var height = width / 2.5;
            var chart = d3Charts.lineChart(animation, width, height)
                .labels(labels)
        ...
        var updateAreaGraph = function(d) {
            var $problemCharArea = wrapper.find('.problemarea-chart-area');
            $problemCharArea.find('svg').remove();
            $problemCharArea.prepend('<svg class="donut-chart"></svg>');
        ...
        this.init = function(wrap, util, params, callback) {
            ...
            viewModel = {
                ...
                showCustomCalendar: function() {
                    ...
                    wrapper.find(pikerContainer).show();
                    var hidePikerHandler = function () {
                        viewModel.changePeriod(false);
                        wrapper.find(pikerContainer).hide();
            ...
            utility.asyncParallel([
                ...
                viewModel.comparativeTerm.subscribe(refreshDashboard);
                ...
                var maxDate = new Date(new Date().toUTCString());
                ...
                pikerStart = pikerStartField ? new pikaday({
                    ...
                    onSelect: function(date) {
                        var label = utility.resx.CommunityAnalytics.label_From + ':&nbsp;&nbsp;&nbsp;' + utility.serializeCustomDate(date);
                        viewModel.startDateLabel(label);
                        pikerEnd.setMinDate(date);
                        pikerEnd.draw();
                        if (date.getTime() !== viewModel.initialStartDate.getTime()) {
                ...
                pikerEnd = pikerEndField ? new pikaday({
                    ...
                    maxDate: maxDate,
                    onSelect: function(date) {
                        var label = utility.resx.CommunityAnalytics.label_To + ':&nbsp;&nbsp;&nbsp;' + utility.serializeCustomDate(date);
                        viewModel.endDateLabel(label);
                        pikerStart.setMaxDate(date);
                        pikerStart.draw();
                        if (date.getTime() !== viewModel.initialEndDate.getTime()) {
                            wrapper.find('.pika-calendar-apply-btn').removeClass('disabled');
                        }
                    }
                }) : null;
                ...
                $('.pika-select').attr('aria-label', 'Date');
