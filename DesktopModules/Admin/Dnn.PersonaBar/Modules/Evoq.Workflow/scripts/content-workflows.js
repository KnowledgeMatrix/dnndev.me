...
define([
    'jquery',
    'knockout',
    'main/PersonaBarDialog',
    './content-workflows-states',
    'knockout.validation.min',
    'jquery-ui.min',            // Color effect
    'jquery.qatooltip',
	'dnn.jquery'],

function ($, ko, PersonaBarDialog, ContentWorkflowsStates) {
    ...
    ContentWorkflowClass = (function IIFE() {
        ...
        var _browser, _koBinded, htmlHeader, configServiceFramework, configServiceFrameworkStates, util, settings, viewModel, newWorkflowExtendedObj, viewModelExtra, isMobile, newWorkflowJqObj, newWorkflowJqObjTitle, newWorkflowJqObjAnimationPanel, personaBarDialog,
        ...
        /* Constructor */
        function ContentWorkflow(wrapper, utilities, params, callback) {
        ...
        initViewModel = function (data, htmlHeader) {
            ...
            viewModel = {
              browser:    _browser,
                ...
                defaultWorkflow: ko.observable(0),
                setDefaultWorkflow: function () {
                    var workflowId = this.WorkflowId();

                    if (workflowId === viewModel.defaultWorkflow()) {
                        return;
                    }

                    configServiceFramework();
                    util.sf.post('SetDefaultWorkflow?workflowId=' + workflowId, {}, function(data) {
                        viewModel.defaultWorkflow(workflowId);
                    }, function() {
                        util.notifyError(_resx.workflows_unknown_error);
                    });
                },
                workflows: {
                    header: {
                        ...
                        isDefault: _resx.workflows_header_isDefault || 'DEFAULT'
                    },
                    tooltip: {
                ...
                moveDown:           moveDown,
                canEdit: function() {
                    return settings.isHost || settings.isAdmin || (settings.permissions && settings.permissions.EDIT === true);
                }
            };
            ...
            data.Workflows.forEach(function (elemWorkFlow, idx) {
                item = {
                    ...
                    isDefault: ko.computed(function () {
                        return elemWorkFlow.WorkflowId === viewModel.defaultWorkflow();
                    })
                };
            ...
            viewModel.defaultWorkflow(data.DefaultWorkflow);
                        ...
                        '<div class="resourcesUsedByWorkflow resources">' +
                            '<ul class="" data-bind="foreach: messages">'+
                                '<li class="clearBoth" data-bind="css: {odd: $index()%2 === 0, even: $index()%2 === 1}">' +
                                    ...
                                    '<span class="contentType" data-bind="text: contentType, attr: { title: contentType }"></span>' +
        ...
        initErrorCallback = function (data) {
            if (!data) util.notifyError(_resx.workflows_unknown_error);
            else if (data.status !== 200) util.notifyError('Error ' + data.status + ': ' + data.statusText);
        ...
        animateShowPanel = function (isEditable, objTitle, objPanel, numberOfStates, endAnimationCallback) {
            ...
            finalHeight = objPanel.find('.lastTag')[0].getBoundingClientRect().bottom - objPanel[0].getBoundingClientRect().bottom + LAST_TAG_TO_END_DISTANCE;
        ...
        createWorkflow200Callback = function (data) {
            ...
            newWorkflow = {
                ...
                DescriptionErrors: ko.observableArray([]),
                isDefault: ko.computed(function () {
                    return data.Workflow.WorkflowId === viewModel.defaultWorkflow();
                })
            };
            ...
            hideCallback = function () {
                ...
                highlightLastAdded();
                showEdit(newWorkflow, viewModel.workflows.list().length -1);
            };
        ...
        showEdit = function (target, index) {
            ...
            _tmpEditedWorkflow.target        = target;
        };
        ...
        hideEdit = function (index, force) {
            ...
            if (index && index === _tmpEditedWorkflow.index) {
                unbindDescriptionHeight(_tmpEditedWorkflow.target);
                cancelEdited(viewModel.workflows.list()[index], _tmpEditedWorkflow.target, index);
            }
        ...
        toggleEdit = function (workflow, event, index) {
            if (index === 0) {
                ...
                if (viewModel.isEditOpen()[index]) {
                    cancelEdited(workflow, event.target, index);
                    hideEdit(index);
                } else {
                    showEdit(event.target, index);
                }
            }
        };
        ...
        editWorkflowErrorCallback = function (data, workflow, eventTarget) {
            if (data && data.status != 500 && data.responseJSON && data.responseJSON.Messages) {
                ...
                util.notifyError(_resx.unknown_error);
            }
        };
        ...
        delWorkflowErrorCallback = function (data) {
            ...
            */
            ...
            if (!data.responseJSON || !data.responseJSON.Messages) {
                util.notifyError('Error ' + data.status + ': ' + data.statusText);
            }
        ...
        cleanEditedTmp = function() {
            ...
            _tmpEditedWorkflow.target        = null;
        };

        cancelEdited = function (workflow, target, index) {
            workflow.WorkflowName(_tmpEditedWorkflow.WorkflowName);
            workflow.Description(_tmpEditedWorkflow.Description);

            cleanEditedTmp();

            cleanBackendErrors(workflow, target);
            hideEdit(index);
        };
