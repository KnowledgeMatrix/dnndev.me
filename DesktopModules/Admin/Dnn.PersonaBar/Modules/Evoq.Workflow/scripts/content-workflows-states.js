...
define([
	...
	'css!main/../css/permissionGrid.css',
	'main/permissionGrid',
...
function ($, ko, PersonaBarDialog) {
	...
	ContentWorkflowsStatesClass = (function IIFE() {
		...
		_html = '<div class="panel firstPanel workflowState">\
					...
					<input class="stateName" type="text" data-bind="value: stateName, valueUpdate: \'afterkeydown\', event: {blur: validateStateName}" aria-label="Name" >\
				...
				<div class="panel">\
					<input type="checkbox" data-bind="checked: chboxNotifAuthorVal" aria-label="Notify Author"> <span data-bind="text: chboxNotifAuthorTxt"></span><br><br>\
					<input type="checkbox" data-bind="checked: chboxNotifAdminVal" aria-label="Notify Admin"> <span data-bind="text: chboxNotifAdminTxt"></span>\
				</div>';
		...
		configServiceFramework = function () {
			_util.sf.moduleRoot = 'PersonaBar';
		...
		afterBind200Callback = function (data) {
			...
			dnn.controls.PermissionGrid.resx = _util.resx.PersonaBar;
    ...
    editAndAdderrosCallback = function(data) {
        var messages;
        _saved = false;
        if (!data) _util.notifyError(_resx.workflows_unknown_error);
        if (data.status === 500) _util.notifyError('Error ' + data.status + ': ' + data.statusText);
